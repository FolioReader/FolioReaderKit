...
class FolioReaderPlayerMenu: UIViewController, SMSegmentViewDelegate, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        menuView = UIView(frame: CGRect(x: 0, y: view.frame.height - (115 + safeAreaInsetsBottom), width: view.frame.width, height: view.frame.height + safeAreaInsetsBottom))
...
//        styleOptionBtns.append(style2)
    ...
    @objc func changeStyle(_ sender: UIButton!) {
        ...
        if let webView = self.folioReader.readerCenter?.currentPage?.webView {
            let script = "setMediaOverlayStyle(\"\(self.folioReader.currentMediaOverlayStyle.className())\")"
